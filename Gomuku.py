...
def is_bounded(board, y_end, x_end, length, d_y, d_x):for i in range(length):
        board[y][x] = col
        y += d_y
        x += d_x
    while True:
...
def play_gomoku(board_size):
    ...
    for i in range(length):
        ...
        while True:
            ...
            print("Computer move: (%d, %d)" % (move_y, move_x))
            board[move_y][move_x] = "b"for i in range(length):
        board[y][x] = col
        y += d_y
        x += d_x
    while True:
...
def is_sequence_complete(board, col, y_start, x_start, length, d_y, d_x):
    for i in range(length):
        if is_sq_in_board(board, y_start,x_start) == True:
